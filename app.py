...
def pagina_ingreso():
    ...
    with col2:
        ...
        if prod_info:
            marca_str = prod_info.get('marca') or ''
            conc_str = prod_info.get('concentracion') or ''
            extra = " | ".join(filter(None, [marca_str, conc_str]))
            caption_parts = [prod_info.get('presentacion', 'Sin presentación'), prod_info.get('unidad_medida', 'unidad')]
            if extra:
...
def pagina_egreso():
    ...
    with col2:
        ...
        if prod_info_egr:
            marca_egr = prod_info_egr.get('marca') or ''
            conc_egr = prod_info_egr.get('concentracion') or ''
            extra_egr = " | ".join(filter(None, [marca_egr, conc_egr]))
            if extra_egr:
